...
import { useGetWabaPhoneNumbersQuery, useDisconnectWhatsAppMutation, useSetPrimaryPhoneNumberMutation } from "@/src/redux/api/whatsappApi";
...
const PhoneNumbers = ({ wabaId }: PhoneNumbersProps) => {
  ...
  const [primaryTarget, setPrimaryTarget] = useState<WabaPhoneNumber | null>(null);
  ...
  const [setPrimaryPhone, { isLoading: isSettingPrimary }] = useSetPrimaryPhoneNumberMutation();
  ...
  const handleSetPrimary = async () => {
    ...
    try {
      ...
      if (response.success) {
        toast.success(response.message || "Phone number marked as primary successfully");
