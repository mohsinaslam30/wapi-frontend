...
import { ChevronLeft, ChevronRight, LayoutGrid, Save, Search, Workflow, X, Send, Play, Sparkles } from "lucide-react";
...
const FlowCanvas = () => {
  ...
  const restoreFlowFromData = useCallback((flow: any) => {
      ...
      const restoredNodes = flow.nodes
        .filter((n: any) => {
          ...
          if (n.type === "condition" && (n.id.startsWith("cond-start-") || n.id.startsWith("cond-btn-") || n.id.startsWith("cond-list-"))) return false;
          return true;
        })
        .map((n: any) => {
          ...
          return {
            ...
            data: {
              ...
              message: params.message || params.message_template || params.message_body || "",
              keywords: keywords || [],
  ...
  const handleSave = async (isActive: boolean = true) => {
    try {
      ...
      if (flowId) {
        ...
        toast.success(isActive ? "Flow published successfully!" : "Flow saved in draft!");
      } else {
          ...
          <div className="flex items-center gap-2">
            ...
            {!flowData?.data?.is_active && (
              ...
              >
                ...
                <span className="text-[10px] md:text-xs">{isCreating || isUpdating ? "Wait..." : "Save Draft"}</span>
              </Button>
            )}
            <Button
            ...
            >
              ...
              <span className="text-[10px] md:text-xs">{isCreating || isUpdating ? "Wait..." : "Publish"}</span>
            </Button>
          </div>
        </div>
