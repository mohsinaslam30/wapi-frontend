        ...
        <div className="flex-1 min-w-0 flex flex-col justify-center gap-0.5 ms-0">
          <div className="flex justify-between items-center pe-2">
            ...
            <div className="flex items-center gap-1">
              {!isSelectionMode && (
                <>
                  <span className={cn("text-[11px] whitespace-nowrap", lastMessage.unreadCount && lastMessage.unreadCount !== "0" ? "text-emerald-600 font-bold" : "text-slate-500 dark:text-gray-400")}>{lastMessage.createdAt ? new Date(lastMessage.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "Now"}</span>
          ...
          <div className="flex justify-between items-center gap-2">
            <p className={cn("text-xs truncate max-w-45", lastMessage.unreadCount && lastMessage.unreadCount !== "0" ? "text-slate-900 dark:text-white font-medium" : "text-slate-500 dark:text-gray-500")}>{getMessagePreview()}</p>
